  ...
  const { error } = await getResend().emails.send({
    ...
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 520px; margin: 0 auto; padding: 24px;">
        ...
        <p style="color: #475569; margin-bottom: 8px;">Hi ${businessName},</p>
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
          ...
          <p style="margin: 0 0 12px; font-size: 18px; font-weight: 600; color: #0f172a;">${deadlineName}</p>
          <p style="margin: 0; font-size: 14px; color: #64748b;">Due: <strong>${dueDate}</strong></p>
        </div>

        <a href="${deadlineUrl}" style="display: inline-block; background: #2563eb; color: white; font-weight: 600; font-size: 14px; padding: 12px 24px; border-radius: 10px; text-decoration: none; margin-bottom: 24px;">
          View deadline →
        </a>
